...
void GraphManager::sendNewCloud(Node* node) {
    ScopedTimer s(__FUNCTION__);

    if (new_cloud_pub_.getNumSubscribers() == 0){
      ROS_WARN("No Subscribers: Sending of new cloud cancelled");
      return;
    }

    ROS_INFO("Sending out new clouds");
